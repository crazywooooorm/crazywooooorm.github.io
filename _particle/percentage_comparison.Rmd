...
How big is the difference between the two tests? We did a simulation to compare the test result between using Chi-squared test and Student's t-test.
...
As we can see from the simulation result, when the sample sizes are small, t-test is easier to reject the null hypothesis; while when the sample sizes are big enough in both groups, the test results from those two tests tend to be same.
